...
/*Global Variables */
int jj;
double Iion,Ist,Ina,Ik1,Ito,Ikur,Ikr,Iks,Ical,Iup;
...
Ikr = gkr*rN.xr*(rN.V-Ek)/(1.0 + shaisExp((rN.V+15.0)/22.4));
Iks = gks*rN.xs*rN.xs*(rN.V-Ek);
Ical = gcal*rN.d*rN.f*rN.fca*(rN.V-65.0);
Inak = Inakmax*fnak*(1.0/(1.0+ pow(Kmnai/rN.Nai,1.5))) * Ko/(Ko+Kmko);
Inaca = Inacamax*(shaisExp(lambda*rN.V/(RTF))*rN.Nai*rN.Nai*rN.Nai*Cao - shaisExp((lambda-1)*rN.V/(RTF))*Nao*Nao*Nao*rN.Cai)/
((Kmna*Kmna*Kmna + Nao*Nao*Nao)*(Kmca + Cao)*(1 + ksat*shaisExp((lambda-1)*rN.V/(RTF))));
Ibca = gbca*(rN.V-Eca);
Ibna = gbna*(rN.V-Ena);
...
ach2 = (0.0517 + 0.4516/(1.0 + shaisExp((rN.V+59.53)/17.18)));	// for [ACH]=1uM only!  Kneller 2002
...
am = 0.32*(rN.V+47.13)/(1-shaisExp(-0.1*(rN.V+47.13)));
}
bm = 0.08*shaisExp(-rN.V/11.0);
taum = 1.0/(am+bm);
infm = am*taum;
...
ah = 0.135*shaisExp( -1.0*(rN.V+80.0)/6.8);
}
if(rN.V>=-40.0){
bh = 1.0/(0.13*(1+shaisExp(-1.0*(rN.V+10.66)/11.1)));
}else{
bh = 3.56*shaisExp(0.079*rN.V) + (3.1e5)*shaisExp(0.35*rN.V);
}
tauh = 1.0/(ah+bh);
infh = ah*tauh;
...
aj = ( -127140.0*shaisExp(0.2444*rN.V) - (3.474e-5)*shaisExp(-0.04391*rN.V) )*(rN.V+37.78)/
        (1.0+shaisExp(0.311*(rN.V+79.23)));
}
if(rN.V>=-40.0){
bj = 0.3*shaisExp((-2.535e-7 )*rN.V)/(1.0+shaisExp(-0.1*(rN.V+32.0)));
}else{
bj = 0.1212*shaisExp(-0.01052*rN.V)/(1.0+shaisExp(-0.1378*(rN.V+40.14)));
}
tauj = 1.0/(aj+bj);
infj = aj*tauj;

asa = 0.65/( (shaisExp(-1.0*(rN.V+10.0)/8.5)) + (shaisExp(-1.0*(rN.V-30.0)/59.0)) );
bsa = 0.65/(2.5 + shaisExp((rN.V+82.0)/17.0) );
infsa = 1.0/(1.0 + shaisExp(-1.0*(rN.V+20.47)/17.54));
tausa = (1.0/(asa+bsa))/Kq10;

asi = 1.0/(18.53 + shaisExp((rN.V+113.7)/10.95));
bsi = 1.0/(35.56 + shaisExp(-1.0*(rN.V+1.26)/7.44));
infsi = 1.0/(1.0 + shaisExp((rN.V+43.1)/5.3));
tausi = (1.0/(asi+bsi))/Kq10;

gkur = gkur_sub*(0.005 + 0.05/(1.0+shaisExp(-1.0*(rN.V-15.0)/13.0)));
aua = 0.65/(shaisExp(-1.0*(rN.V+10.0)/8.5) + shaisExp(-1.0*(rN.V-30.0)/59.0));
bua = 0.65/(2.5+shaisExp((rN.V+82.0)/17.0));
infua = 1.0/(1.0 + shaisExp(-1.0*(rN.V+30.3)/9.6)); 
tauua = (1.0/(aua+bua))/Kq10;

aui = 1.0/(21.0 + shaisExp(-1.0*(rN.V-185.0)/28.0));
bui = shaisExp((rN.V-158.0)/16.0);
infui = 1.0/(1.0 + shaisExp((rN.V-99.45)/27.48)); 
tauui = (1.0/(aui+bui))/Kq10;

axr = 0.0003*(rN.V+14.1)/(1.0 - shaisExp(-1.0*(rN.V+14.1)/5.0));
bxr = (7.3898e-5)*(rN.V-3.3328)/(shaisExp((rN.V-3.3328)/5.1237)-1.0);
infxr = 1.0/(1.0 + shaisExp(-1.0*(rN.V+14.1)/6.5)); 
/*tauxr = (1.0/(axr+bxr))/Kq10;*/
tauxr = (1.0/(axr+bxr));

axs = (4e-5)*(rN.V-19.9)/(1.0 - shaisExp(-1.0*(rN.V-19.9)/17.0));
bxs = (3.5e-5)*(rN.V-19.9)/(shaisExp((rN.V-19.9)/9.0) - 1.0);
tauxs = 0.5/(axs+bxs);
infxs = 1.0/sqrt(1.0 + shaisExp(-1.0*(rN.V-19.9)/12.7));

taud = (1.0 - shaisExp(-1.0*(rN.V+10.0)/6.24))/(0.035*(rN.V+10.0)*(1.0+shaisExp(-1.0*(rN.V+10.0)/6.24)));
infd = 1.0/(1.0 + shaisExp(-1.0*(rN.V+10.0)/8.0));

tauf = 9.0/(0.0197*shaisExp(-1.0*0.0337*0.0337*(rN.V+10.0)*(rN.V+10.0)) + 0.02);
inff = 1.0/(1.0 + shaisExp((rN.V+28.0)/6.9));

inffca = 1.0/(1.0+rN.Cai/0.00035);

sigma = (1.0/7.0)*(shaisExp(Nao/67.3) - 1.0);
fnak = 1.0/(1.0 + 0.1245*shaisExp(-0.1*rN.V/(RTF)) + 0.0365*sigma*shaisExp(-1.0*rN.V/(RTF)));

infu = 1.0/(1.0+ shaisExp(-1.0*(Fn - 3.4175e-13)/13.67e-16));

tauv = 1.91 + 2.09/(1.0 + shaisExp(-1.0*(Fn - 3.4175e-13)/13.67e-16));
infv = 1.0 - 1.0/(1.0 + shaisExp(-1.0*(Fn - 6.835e-14)/13.67e-16));

tauw = 6.0*(1.0 - shaisExp(-1.0*(rN.V-7.9)/5.0))/( (1.0 + 0.3*shaisExp(-1.0*(rN.V-7.9)/5.0))*(rN.V-7.9) );
infw = 1.0 - 1.0/(1.0 + shaisExp(-1.0*(rN.V-40.0)/17.0));
